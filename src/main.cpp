...
void tft_disp(const char* mode, float dist = 0, float cal = 0, int steps = 0) {
  TFT.background(Black); // Clear the screen with black color
  TFT.foreground(White);
  TFT.set_font((unsigned char*) Arial28x28);
  TFT.locate(15, 130);
  TFT.printf("Mode: %s", mode);
  // Display dist, cal, and steps if needed
}
...
int main()
{
  ...
  tft_init();
  while (1) {
    ThisThread::sleep_for(0.1);  // Add a wait to control the loop rate
    gyro_read();
...
bool compare_sequence() {
  bool match = true;
  int time_to_wait = sequence_length * 0.1; // Calculate the total time to wait based on the recorded sequence length
  for (int i = 0; i < sequence_length; i++) {
    ThisThread::sleep_for(0.1); // Wait before reading the next data point
  ...
  for (int i = 0; i < (time_to_wait - sequence_length); i++) {
